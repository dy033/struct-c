...
{
    int tes;
    extern char name[MAXSIZE];
    extern char id[MAXSIZE];
    extern char passwd[MAXSIZE];
    extern char booknum[MAXSIZE][MAXSIZE];
    extern int bookn;           //记录借的书的种数
    extern int booknu[MAXSIZE]; //不同种书的数量
} INF;
INF *inf[MAXSIZE];
int logout = 0;
int main();
