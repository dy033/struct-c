...
int lend(LinkList &L, char nu[MAXSIZE], int i, INF *inff, int r4) //通过书号借阅书籍
{
    LinkList pre;
    int k;
    pre = L;
    ...
    printf("成功借出此书!\n");
    strcpy(inff[r4]->booknum[inff[r4]->bookn], pre->num);
    inff[r4]->booknu[inff[r4]->bookn] += i;
    inff[r4]->bookn++;
    return 1;
}
int lend1(LinkList &L, char nam[MAXSIZE], int i, INF *inff, int r4) //通过书名借出书籍
{
    LinkList pre;
    int k;
    pre = L;
    ...
    printf("成功借出此书!\n");
    strcpy(inff[r4]->booknum[inff[r4]->bookn], pre->num);
    printf("%s", inff[r4]->booknum[inff[r4]->bookn]);
    inff[r4]->booknu[inff[r4]->bookn] += i;
    inff[r4]->bookn++;
    return 1;
}
...
int returnb(LinkList &L, char nu[MAXSIZE], int i, INF *inff, int r4) //通过书号归还书籍
{
    LinkList pre;
    int k;
    pre = L;
    ...
    printf("%s\t%s\t%s\t%d\t%d\n", pre->num, pre->name, pre->author, i, pre->now);
    printf("成功归还此书!\n");
    strcpy(inff[r4]->booknum[inff[r4]->bookn], "");
    inff[r4]->booknu[inff[r4]->bookn] -= i;
    inff[r4]->bookn--;
    return 1;
}
int returnb1(LinkList &L, char nam[MAXSIZE], int i, INF *inff, int r4) //通过书名归还书籍
{
    LinkList pre;
    int k;
    pre = L;
...
//     return 1;
// }
int own(LinkList &L, INF *inff, int r3)
{
    LinkList pre;
    int k = 0;
    pre = L;
    ...
    {
        pre = pre->next;
        printf("%s", inff[r3]->booknum[inff[r3]->bookn]);
        if (!strcmp(pre->num, inff[r3]->booknum[inff[r3]->bookn]))
            printf("%s\t%s\t%s\t%d\t%d\n", pre->num, pre->name, pre->author, inff[r3]->booknu[inff[r3]->bookn], pre->all);
        k++;
    }
    if (k == 0)
    {
...
void switchl(LinkList &M, INF *inf1, int r3)
{
    int flag = 0;
    do
    {
...
void switchr(LinkList &M, INF *inf1, int r3)
{
    int flag = 0;
    do
    {
