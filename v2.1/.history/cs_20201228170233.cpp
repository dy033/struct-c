...
int bookdatar(LinkList &L) //读取存储图书馆中所有书籍的数据
{
    ...
    while (!feof(fp))
    {
        ...
        while (datatemp != NULL)
        {
            datatemp = strtok(data, ",");
            s->bookflag = atoi(datatemp);
            s->num = strtok(NULL, ",");
            s->name = s->num = strtok(NULL, ",");
            s->author = s->num = strtok(NULL, ",");
            datatemp = strtok(NULL, ",");
            s->now = atoi(datatemp);
            datatemp = strtok(NULL, ",");
            s->all = atoi(datatemp);
        }
...
int userdatar(int r3) //读取用户借阅所有书籍数据
{
    ...
    while (!feof(fp))
    {
        fscanf(fp, "%d", &inf[r3].bookn);
        fscanf(fp, "%s%d%d%d%d", inf[r3].booknum[inf[r3].bookn], &inf[r3].booknu[inf[r3].bookn], &inf[r3].y[inf[r3].bookn], &inf[r3].m[inf[r3].bookn], &inf[r3].d[inf[r3].bookn]);
        // bookn++;
    }
    fclose(fp);
    return 1;
