...
int lend(LinkList &L, char nu[MAXSIZE], int i, int r4) //通过书号借阅书籍
{
    ...
    printf("成功借出此书!\n");
    inf[r4].booknum[inf[r4].bookn] = pre->num;
    inf[r4].bookn++;
    return 1;
}
...
void own(LinkList &L, int r3)
{
    ...
    pre = L;
    k = 0;
    while (pre->next != NULL && strcmp(pre->name, num))
    {
        pre = pre->next;
    }
    if (!(pre))
...
void initinf() //添加5个用户
{
    ...
    inf[0].tes = 1; //查看是否有用户
    inf[0].bookn = 0； inf[1].bookn = 0； inf[2].bookn = 0； inf[3].bookn = 0； inf[4].bookn = 0；
}
