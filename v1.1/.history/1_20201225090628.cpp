...
int bookdataw(LinkList &L) //存储图书馆中所有书籍数据
{
    ...
    fp = fopen("/book/bookdata.txt", "w");
    while (pre->next != NULL)
    {
        pre = pre->next;
        fprintf(fp, "%d\t %s\t %s\t %s\t  %d\t  %d\n", pre->bookflag, pre->num, pre->name, pre->author, pre->now, pre->all);
    }
    fclose(fp);
    return 1;
}
...
int userdatar(LinkList &L, int r3) //读取用户借阅所有书籍数据
{
    LinkList pre;
    pre = L;
    FILE *fp;
    ...
    while (!feof(fp))
    {
        fscanf("%d\t ", inf[r3].bookn);
        fscanf(fp, " %s\t %d\t\t%d年%d月%d日\n", inf[r3].booknum[inf[r3].bookn], inf[r3].booknu[inf[r3].bookn], inf[r3].y[inf[r3].bookn], inf[r3].m[inf[r3].bookn], inf[r3].d[inf[r3].bookn]);
    }
    fclose(fp);
    return 1;
}
